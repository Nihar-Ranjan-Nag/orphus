...
function FaqItem({ q, a, idx }: { q: string; a: string; idx: number }) {
  ...
  return (
    ...
    >
      ...
      >
        <span className="font-display font-semibold text-[18px] text-[#15231F] group-hover:text-gold transition-colors leading-snug">
          {q}
        </span>
        <span className={`flex-none flex h-8 w-8 items-center justify-center rounded-full border transition-all duration-300 ${
          open ? "border-gold bg-gold text-charcoal" : "border-black/15 text-[#15231F]/50 group-hover:border-gold group-hover:text-gold"
        }`}>
          {open ? <Minus size={14} /> : <Plus size={14} />}
        </span>
      </button>
      <div ref={bodyRef} style={{ height: 0, overflow: "hidden", opacity: 0 }}>
        <p className="pb-6 font-body text-[16px] text-[#15231F] leading-[1.85] max-w-3xl font-bold">{a}</p>
      </div>
    </div>
  );
}
...
export function FaqSection() {
  ...
  return (
    <section className="bg-white py-28 md:py-36 relative overflow-hidden">
      <FloatingParticles count={45} color="rgba(11,93,77,0.35)" ringColor="rgba(11,93,77,0.2)" />
      ...
      <div className="relative mx-auto max-w-[1280px] px-6 md:px-16 grid lg:grid-cols-[1fr_2fr] gap-16 lg:gap-20">
        ...
        <div className="lg:pt-2">
          <p className="font-display text-[13px] font-semibold uppercase tracking-[3px] text-gold">FAQ</p>
          ...
          <p className="mt-5 font-body text-[16px] leading-relaxed max-w-xs font-bold" style={{ color: "rgba(29,28,28,0.85)" }}>
            Can't find your answer? Email us at{" "}
            <a href={EMAIL_HREF} className="text-gold hover:underline">
              {CONTACT.email}
            </a>
          </p>
        </div>
