import { CONTACT, PHONE_HREF, EMAIL_HREF } from "@/data/contact";
  ...
  return (
    <div className="relative mx-auto select-none" style={{ width: 400, height: 400 }}>
      ...
      <div className="absolute inset-0 rounded-full pointer-events-none"
        style={{ boxShadow: "0 0 80px rgba(11,93,77,0.18), 0 0 0 1px rgba(11,93,77,0.12)" }} />
      ...
      <svg viewBox="0 0 400 400" width={400} height={400} className="overflow-visible">
        <defs>
          ...
          <radialGradient id="atmo" cx="38%" cy="30%" r="60%">
            <stop offset="0%"   stopColor="rgba(198,164,90,0.08)" />
            <stop offset="100%" stopColor="transparent" />
          </radialGradient>
          {/* sphere base */}
          <radialGradient id="sphere-bg" cx="40%" cy="35%" r="65%">
            <stop offset="0%"   stopColor="#0A2F28" />
            <stop offset="55%"  stopColor="#071F1B" />
            <stop offset="100%" stopColor="#061512" />
          </radialGradient>
          {/* connection line gradient */}
          <linearGradient id="line-grad" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%"   stopColor="#0B5D4D" stopOpacity="0.8" />
            <stop offset="100%" stopColor="#0B5D4D" stopOpacity="0.15" />
          </linearGradient>
        </defs>
        ...
        <g clipPath="url(#globe-clip)">
          ...
          {PARALLELS.map((d, i) => (
            <path key={`par-${i}`} d={d} fill="none"
              stroke="rgba(11,93,77,0.18)" strokeWidth="0.7" />
          ))}
          {MERIDIANS.map((d, i) => (
            <path key={`mer-${i}`} d={d} fill="none"
              stroke="rgba(11,93,77,0.13)" strokeWidth="0.7" />
          ))}
          ...
          {CITIES.filter(c => !c.hq).map((c) => {
            ...
            return (
              <path key={`arc-${c.name}`}
                ...
                fill="none" stroke="rgba(11,93,77,0.35)" strokeWidth="0.9"
                strokeDasharray="3 4" />
            );
          })}
          ...
          {CITIES.map((c) => (
            <g key={c.name}>
              ...
              <circle cx={c.x} cy={c.y} r={c.hq ? 9 : 6} fill="none"
                stroke={c.hq ? "rgba(11,93,77,0.5)" : "rgba(198,164,90,0.35)"}
                strokeWidth="1">
              ...
              <circle cx={c.x} cy={c.y} r={c.hq ? 4.5 : 3}
                fill={c.hq ? "#0B5D4D" : "#C6A45A"}
                style={{ filter: c.hq ? "drop-shadow(0 0 5px #0B5D4D)" : "drop-shadow(0 0 3px #C6A45A)" }} />
            </g>
          ))}
        </g>
        ...
        <circle cx={CX} cy={CY} r={R} fill="none"
          stroke="rgba(11,93,77,0.22)" strokeWidth="1" />
          ...
          return (
            <g key={`lbl-${c.name}`}>
              ...
              <text x={c.x + dx} y={c.y + 8}
                ...
                fill="rgba(198,164,90,0.7)" textAnchor={anchor as "start" | "end"}>
                {c.sub}
              </text>
            </g>
          );
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-2 rounded-full px-4 py-1.5 pointer-events-none"
        style={{
          ...
          border: "1px solid rgba(11,93,77,0.25)",
          boxShadow: "0 4px 20px rgba(0,0,0,0.4)",
        }}>
        <span className="h-1.5 w-1.5 rounded-full bg-[#0B5D4D]"
          style={{ boxShadow: "0 0 6px #0B5D4D" }} />
        <span className="font-display font-semibold text-[12px] uppercase tracking-[2px] text-white/70">
          18 Active Markets
        </span>
      </div>
    </div>
...
const INPUT_STYLE: React.CSSProperties = {
  ...
  border: "1px solid rgba(11,93,77,0.2)",
  borderRadius: "12px",
  padding: "12px 16px",
  width: "100%",
  color: "#fff",
...
export function HomeContact() {
  ...
  const focusStyle = (name: string): React.CSSProperties => ({
    ...INPUT_STYLE,
    borderColor: focused === name ? "rgba(11,93,77,0.6)" : "rgba(11,93,77,0.2)",
    boxShadow: focused === name ? "0 0 0 3px rgba(11,93,77,0.08)" : "none",
  });
  ...
  return (
    <section ref={sectionRef} className="relative overflow-hidden py-24 md:py-32"
      style={{ background: "#061512" }}>

      {/* grid texture */}
      <div className="absolute inset-0 pointer-events-none opacity-[0.03]" style={{
        backgroundImage: "linear-gradient(rgba(11,93,77,1) 1px, transparent 1px), linear-gradient(90deg, rgba(11,93,77,1) 1px, transparent 1px)",
        backgroundSize: "64px 64px",
      }} />
      {/* top edge */}
      <div className="absolute inset-x-0 top-0 h-px" style={{
        background: "linear-gradient(90deg, transparent, rgba(11,93,77,0.3), transparent)"
      }} />
      ...
      <div className="relative z-10 mx-auto max-w-[1280px] px-6 md:px-16">
        <div className="grid lg:grid-cols-2 gap-16 xl:gap-24 items-center">
          ...
          <div>
            <div data-ca>
              <p className="font-display text-[13px] font-semibold uppercase tracking-[3px]"
                style={{ color: "#0B5D4D" }}>
                Get In Touch
              </p>
              <h2 className="mt-4 font-display font-extrabold leading-[1.0] text-white"
                ...
                <span className="font-serif italic" style={{
                  background: "linear-gradient(130deg, #C6A45A 0%, #D7BC78 50%, #0B5D4D 100%)",
                  WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text",
                }}>
                  Your Future.
                </span>
              </h2>
              <p className="mt-4 font-body text-[17px] leading-relaxed font-bold"
            ...
            <div data-ca className="mt-7 flex flex-wrap gap-3 needs-copy" data-marker="Copy: confirm address / phone / email">
              {[
                ...
                { icon: <Phone size={12} />,  text: CONTACT.phoneDisplay, href: PHONE_HREF },
                { icon: <Mail size={12} />,   text: CONTACT.email, href: EMAIL_HREF },
              ].map(p => (
                p.href
                  ? <a key={p.text} href={p.href}
                      target={p.href.startsWith("http") ? "_blank" : undefined}
                      rel={p.href.startsWith("http") ? "noopener noreferrer" : undefined}
                      className="flex items-center gap-2 rounded-full px-4 py-2 font-body text-[14px] transition-all hover:border-gold/40 hover:text-white"
                      style={{ background: "rgba(11,93,77,0.07)", border: "1px solid rgba(11,93,77,0.18)", color: "rgba(255,255,255,0.55)" }}>
                      <span style={{ color: "#C6A45A" }}>{p.icon}</span>{p.text}
                    </a>
                  : <div key={p.text}
                      className="flex items-center gap-2 rounded-full px-4 py-2 font-body text-[14px]"
                      style={{ background: "rgba(11,93,77,0.07)", border: "1px solid rgba(11,93,77,0.18)", color: "rgba(255,255,255,0.55)" }}>
                      <span style={{ color: "#C6A45A" }}>{p.icon}</span>{p.text}
                    </div>
              ))}
            ...
            <div data-ca className="mt-10">
              {sent ? (
                <div className="flex flex-col items-start gap-4 rounded-2xl p-8"
                  style={{ background: "rgba(11,93,77,0.06)", border: "1px solid rgba(11,93,77,0.2)" }}>
                  <CheckCircle2 size={36} style={{ color: "#4ADE80" }} />
                  <div>
                    <p className="font-display font-bold text-[20px] text-white">Message sent!</p>
                    <p className="mt-1 font-body text-[16px]" style={{ color: "rgba(255,255,255,0.45)" }}>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                  ...
                  <button type="submit"
                    className="flex items-center justify-center gap-2.5 rounded-xl py-4 font-display font-semibold text-[15px] uppercase tracking-[2px] text-white transition-all hover:-translate-y-0.5 hover:brightness-110 active:scale-[0.98]"
                    style={{
                      background: "linear-gradient(135deg, #0B5D4D 0%, #08483C 100%)",
                      boxShadow: "0 8px 32px rgba(11,93,77,0.4), inset 0 1px 0 rgba(255,255,255,0.12)",
                    }}>
          ...
          <div data-ca className="flex justify-center lg:justify-end">
            <div ref={footprintRef} className="relative">
              {/* label above */}
              <p data-fp className="text-center font-body text-[12px] uppercase tracking-[3px] mb-6"
                style={{ color: "rgba(255,255,255,0.25)" }}>
                Our Global Footprint
              </p>
              <div data-fp className="site-card rounded-2xl border border-white/10 bg-[#061512] p-2 min-h-[280px]">
                <Suspense fallback={
              ...
              <p
                data-fp
                className="mt-4 text-center font-body text-[14px] uppercase tracking-[2px]"
              ...
              >
                ...
                <a
                  ...
                  className="text-gold underline-offset-2 transition-colors hover:text-[#D7BC78] hover:underline"
                >
                  View on Google Maps
                </a>
              </p>
              ...
              <div data-fp className="mt-6 flex flex-wrap justify-center gap-2 max-w-[400px] mx-auto">
                {["UAE","BVI","Cayman","Hong Kong","Singapore","Mauritius","Seychelles","Luxembourg","Malta","UK","Switzerland","Cyprus"].map(m => (
                  <span key={m}
                    className="rounded-full px-2.5 py-1 font-display text-[11px] uppercase tracking-[1.5px] font-semibold"
                    style={{
                      background: "rgba(11,93,77,0.08)",
                      border: "1px solid rgba(11,93,77,0.15)",
