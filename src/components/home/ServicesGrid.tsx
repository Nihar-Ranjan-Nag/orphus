...
const services: ServiceCard[] = [
  ...
  },
  {
    ...
    href: "/services/private-credit-debt",
  },
];
...
export function ServicesGrid() {
  ...
  return (
    <section ref={sectionRef} className="py-28 md:py-36 relative overflow-hidden" style={{ background: "#F7F4EC" }}>
      ...
      <div className="absolute pointer-events-none" style={{
        ...
        background: "radial-gradient(ellipse at 70% 30%, rgba(11,93,77,0.07) 0%, transparent 65%)",
      }} />

      <div className="relative mx-auto max-w-[1280px] px-6 md:px-16">

        <div className="max-w-3xl">
          <p className="font-display text-[13px] font-semibold uppercase tracking-[3px]" style={{ color: "#0B5D4D" }}>
            Services
          </p>
          <h2
            className="mt-4 font-display font-extrabold leading-[1.0]"
            style={{ fontSize: "clamp(36px, 5vw, 68px)", letterSpacing: "-0.03em", color: "#15231F" }}
          >
          ...
          <p className="mt-5 font-body text-[18px] max-w-xl leading-relaxed font-bold" style={{ color: "rgba(29,28,28,0.85)" }}>
        ...
        <div ref={cardsRef} className="mt-16 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {services.map((s) => (
            <div
              key={s.title}
              data-card
              className="site-card group relative overflow-hidden rounded-3xl cursor-pointer flex flex-col"
              style={{
                background: "#FFFFFF",
                border: "1px solid rgba(11,93,77,0.18)",
            ...
            >
              <div
                ...
                style={{
                  width: "0%",
                  background: "linear-gradient(90deg, #0B5D4D, #C6A45A)",
                }}
              />
              ...
              <span
                ...
                style={{ fontSize: "110px", lineHeight: 1, color: "rgba(11,93,77,0.04)" }}
              >
                {s.n}
              </span>
              ...
              <div className="relative flex flex-col flex-1">
                <div
                  ...
                  style={{
                    background: typeof s.icon === "string" ? "transparent" : "rgba(11,93,77,0.08)",
                    color: "#0B5D4D",
                  }}
                  onMouseEnter={(e) => {
                    if (typeof s.icon !== "string") {
                      (e.currentTarget as HTMLElement).style.background = "#0B5D4D";
                      (e.currentTarget as HTMLElement).style.color = "#ffffff";
                    }
                  }}
                  onMouseLeave={(e) => {
                    if (typeof s.icon !== "string") {
                      (e.currentTarget as HTMLElement).style.background = "rgba(11,93,77,0.08)";
                      (e.currentTarget as HTMLElement).style.color = "#0B5D4D";
                    }
                  }}
                >
                ...
                <h3 className="font-display font-bold text-[20px] md:text-[22px] leading-tight" style={{ color: "#15231F" }}>
                  {s.title}
                </h3>
                <p className="mt-3 font-body text-[16px] leading-[1.8] font-bold flex-1" style={{ color: "rgba(29,28,28,0.65)" }}>
                  {s.body}
                </p>
                ...
                {s.capabilities ? (
                  ...
                  <div className="mt-5 flex flex-wrap gap-2">
                    {s.tags?.map((t) => (
                      <span
                        key={t}
                        className="rounded-full px-3 py-1 font-display text-[12px] uppercase tracking-[1.5px] font-semibold"
                        style={{
                          background: "rgba(11,93,77,0.07)",
                          color: "#08483C",
                          border: "1px solid rgba(11,93,77,0.12)",
                        }}
                      >
                        {t}
                      </span>
                ...
                <Link
                  to={s.href}
                  className="mt-7 inline-flex items-center gap-2 font-display text-[13px] uppercase tracking-[2px] font-semibold transition-all group-hover:gap-3"
                  style={{ color: "#0B5D4D" }}
                >
