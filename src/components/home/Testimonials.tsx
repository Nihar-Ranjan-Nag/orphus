...
export function Testimonials() {
  ...
  return (
    <section ref={sectionRef} className="bg-[#F7F4EC] py-28 md:py-36 relative overflow-hidden">
      <FloatingParticles count={50} color="rgba(11,93,77,0.38)" ringColor="rgba(11,93,77,0.22)" />
      ...
      <div className="absolute top-0 inset-x-0 h-[2px]" style={{
        background: "linear-gradient(90deg, transparent, #0B5D4D, #D7BC78, #0B5D4D, transparent)"
      }} />

      <div className="relative mx-auto max-w-[900px] px-6 md:px-16 text-center">
        <p className="font-display text-[13px] font-semibold uppercase tracking-[3px] text-gold">Testimonials</p>
        <h2 className="mt-4 font-display font-extrabold leading-[1.0]"
          style={{ fontSize: "clamp(32px, 4vw, 54px)", letterSpacing: "-0.03em", color: "#15231F" }}>
          Why Clients{" "}
        ...
        <div ref={slideRef} className="mt-14">
          ...
          <p className="font-serif italic leading-[1.85] text-[#15231F] font-bold"
            style={{ fontSize: "clamp(17px, 2vw, 22px)" }}>
            "{t.quote}"
          </p>

          <div className="mt-10 flex items-center justify-center gap-4">
            <div className="flex h-14 w-14 items-center justify-center rounded-full font-display font-bold text-white text-[17px]"
              style={{ background: "linear-gradient(135deg, #15231F 0%, #061512 100%)" }}>
              {t.initials}
            </div>
            <div className="text-left">
              <div className="font-display font-bold text-[18px] text-[#15231F]">{t.name}</div>
              <div className="font-body text-[14px] text-muted2">{t.role} · {t.company}</div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-10 flex items-center justify-center gap-6">
          <div className="flex items-center gap-2 rounded-full border border-gold/20 bg-white px-4 py-2" style={{ boxShadow: "0 4px 12px rgba(29,28,28,0.03)" }}>
            ...
            <span className="font-display font-semibold text-[14px] text-[#15231F]">5.0</span>
            <span className="font-body text-[13px] text-muted2">Google Rating</span>
          </div>
          <div className="font-body text-[14px] text-muted2">7+ reviews</div>
        </div>
      </div>
    </section>
  );
