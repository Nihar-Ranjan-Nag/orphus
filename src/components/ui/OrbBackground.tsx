  ...
  return (
    <div aria-hidden className="pointer-events-none absolute inset-0 overflow-hidden">
      <div
        ...
        style={{ background: "#0B5D4D", filter: "blur(100px)" }}
      />
      <div
        ref={orb2}
        className="absolute -bottom-40 -left-40 h-[400px] w-[400px] rounded-full opacity-15"
        style={{ background: "#08483C", filter: "blur(80px)" }}
      />
      ...
      {[25, 50, 75].map((p) => (
        <div
          ...
          style={{
            left: `${p}%`,
            background: "linear-gradient(to bottom, transparent, rgba(11,93,77,0.18), transparent)",
          }}
        />
      ))}
    </div>
